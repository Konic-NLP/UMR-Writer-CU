...
function recordPartialGraph_typing(){
    let graphDict = {'n': 1 };
    let graphName = document.getElementById("subgraph_box").value.split(" ")[1];
    let graphHead = document.getElementById("subgraph_box").value.split(" ")[2];
    console.log("selected variable to be partial graph: ", graphHead);
    console.log("partial graph name: ", graphName);
    let k = getKeyByValue(umr, graphHead);
    console.log("key: ", k);
...
function typing_addPartialGraph(){
   let graphName= document.getElementById("subgraph_box").value.split(" ")[3];
   let graphDict = partial_graphs[graphName];
   let role = document.getElementById("subgraph_box").value.split(" ")[2];
   role=role.replace('arg','ARG');
   let snt_id = document.getElementById('curr_shown_sent_id').innerText;
   let cur_parent = document.getElementById("subgraph_box").value.split(" ")[1];
   cur_parent='s'+snt_id.trim()+cur_parent.trim();
   ...
   for(let i = 0; i < sorted_keys.length; i++){
       ...
       if(key.length <=3){//['n', '1.v', '1.s', '1.r', '1.n', '1.c'] root of partial graph
           if(key.endsWith('.c')){
               let headVar;
               if(graphDict[key] !== ""){
                   console.log("graphDict[key]: ", graphDict[key],cur_parent, role, graphDict[key], 'concept')
                   headVar = addTriple(cur_parent, role, graphDict[key], 'concept');
               }else{
                   let stringField = graphDict[key.replace('.c', '.s')];
                   headVar = addTriple(cur_parent, role, stringField, 'string');
               }
               graphDict[key.replace('.c', '.v')] = headVar;
           }
       }else{ //branches of partial graph
